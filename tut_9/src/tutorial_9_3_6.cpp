#include "tutorial_9_3_6.h"

Cylinder::Cylinder() : Node("tutorial_9_3_6")
{
...
bool Cylinder::isThisAWall(const std::vector<geometry_msgs::msg::Point> &segment)
{
    float dist = hypot(segment.front().x - segment.back().x, segment.front().y - segment.back().y);
    return dist > 0.6;
}

bool Cylinder::isThisACorner(const std::vector<geometry_msgs::msg::Point> &segment)
{
